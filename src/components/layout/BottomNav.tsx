...
export function BottomNav() {
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 flex h-16 items-center justify-around border-t border-slate-200/80 bg-white/80 backdrop-blur-md px-2 pb-safe pt-1 shadow-[0_-4px_15px_-3px_rgba(0,0,0,0.05)] dark:border-slate-800/80 dark:bg-slate-900/80 lg:hidden">
...
function BottomNavItem({ to, icon, label }: { to: string; icon: React.ReactNode; label: string }) {
  return (
    <NavLink
      to={to}
      className={({ isActive }) =>
        `flex flex-col items-center justify-center space-y-1 rounded-lg px-2 py-1 transition-all active:scale-95 ${
          isActive
